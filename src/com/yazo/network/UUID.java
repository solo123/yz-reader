...
public final class UUID {
	...
	public static UUID fromString(String name) {
		...
		long mostSigBits = decode(components[0]).longValue();
		mostSigBits <<= 16;
		mostSigBits |= decode(components[1]).longValue();
		mostSigBits <<= 16;
		mostSigBits |= decode(components[2]).longValue();

		long leastSigBits = decode(components[3]).longValue();
		leastSigBits <<= 48;
		leastSigBits |= decode(components[4]).longValue();

		return new UUID(mostSigBits, leastSigBits);
	}

	static String[] split(String buffer,String tag) {

		return StringUtil.split(buffer, tag);
	}
