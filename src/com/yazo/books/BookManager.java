...
import com.yazo.protocol.RequestBookInfo;
...
public class BookManager {
	...
	public LineContent getWebPage1(String page_name) throws IOException,
			XmlPullParserException {
		RequestBookInfo rbi = new RequestBookInfo(
				"http://bk-b.info/reader/pages/" + page_name, "", "GET");
		LineContent lc = null;
		try {
			lc = rbi.getRequestInfo();
		} catch (Exception e) {
	...
	public LineContent getWebPage(String page_name) throws IOException,
			XmlPullParserException {
		// RequestBookInfo rbi = new RequestBookInfo(
		// "http://bk-b.info/reader/pages/" + page_name, "", "GET");
		// rbi.getBookChapterContext();
		HttpConnection conn = (HttpConnection) Connector.open(
				"http://bk-b.info/reader/pages/" + page_name, Connector.READ);
	...
	public LineContent getPageContent(String page_name) throws IOException {
		...
		if (page_name.equals("home")) {
			...
			xml = "<page>\n<header>图书目录</header>\n<content>"
					...
					+ "</content></page>";
		} else if (page_name.startsWith("/book/1?chapter=")
				|| page_name.equals("content")) {
			xml = "<page>\n<header>命运记事本</header>\n<content>"
